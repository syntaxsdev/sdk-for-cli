...
const typesLanguageOption = new Option(
  ...
  .choices(["auto", "ts", "js", "php", "kotlin", "swift", "java", "dart", "python"])
  .default("auto");
...
const typesFlavorOption = new Option(
  "-f, --flavor <flavor>",
  "The flavor/style of types to generate (Python only)"
)
  .choices(["typeddict", "pydantic", "pydantic310"])
  .default("typeddict");

const typesCommand = actionRunner(async (rawOutputDirectory, {language, strict, flavor}) => {
  if (language === "auto") {
  ...
  if (meta.isSingleFile()) {
    const content = templater({
      ...
      getFieldDefinition: meta.getFieldDefinition ? meta.getFieldDefinition.bind(meta) : undefined,
    });
